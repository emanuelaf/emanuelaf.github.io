...
Note that ggplot has a particular default _style_ for plots that includes a gray background and a grid. One may not like it and it is possible to change it by specifying a different _theme_. Check out the  [ggplot2 official website](https://ggplot2.tidyverse.org/reference/ggtheme.html) to browse the different themes or learn [how to make your own](https://emanuelaf.github.io/your-ggplot-theme.html).
...
Here hiding the x-ticks and the x-axis label may be important as we specified just to avoid errors in `ggplot()` but they do not help plot readbility. In order to customise the axis check out the article on [annoying things to do with ggplot](https://emanuelaf.github.io/annoying-ggplot.html).
...
In order to customise the colours check out the article on [annoying things to do with ggplot](https://emanuelaf.github.io/annoying-ggplot.html).
...
To see how to customise colours and how to hide the legend, please refer to the article on [annoying things to do with ggplot](https://emanuelaf.github.io/annoying-ggplot.html).
...
The `geom_smooth()` function adds a smoothing function to the plot. In particular we asked to use a linear model by specifying `lm` as a method and we asked not to show standard errors of the estimates by setting `se = FALSE`. In order to extract the equation of the linear function and to add other types of functions check out the article on [annoying things to do with ggplot](https://emanuelaf.github.io/annoying-ggplot.html).
...
In this post I have given some basics of the _grammar of graphics_. There is a number of packages built on top of it to the extent that a whole website for [ggplot extentions](http://www.ggplot2-exts.org/gallery/) exist! For more advanced ggplot2 and more plots customisation, read the article on [annoying things to do with ggplot](https://emanuelaf.github.io/annoying-ggplot.html).
