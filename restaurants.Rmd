...
Let us first focus on restaurants. The dataset contains the restaurants' id and the type of cusine. It contains info on `r nrow(chefmozcuisine)` different restaurants and there are `r length(unique(chefmozcuisine$Rcuisine))` different types of cuisine. Let us focus only on the 10 most frequent types of cusine:
...
ggplot(data = userprofile) +
  geom_bar(aes(x = activity, fill = activity), col = "black", alpha = 0.5)+
  labs(y = "frequency")+
  theme_bw() +
  theme(legend.position="", axis.text.x=element_text(angle=45, hjust=1))
```
...
```{r}
ggplot(data = userprofile) +
  geom_bar(aes(x = budget, fill = budget), col = "black", alpha = 0.5) +
  labs(y = "frequency")+
  theme_bw() +
  theme(legend.position="", axis.text.x=element_text(angle=45, hjust=1))
```
...
```{r}
ggplot(data = userprofile) +
  geom_bar(aes(x = smoker, fill = smoker), col = "black", alpha = 0.5) +
  labs(y = "frequency")+
  theme_bw() +
  theme(legend.position="", axis.text.x=element_text(angle=45, hjust=1))
```
...
```{r}
ggplot(data = userprofile) +
  geom_bar(aes(x = drink_level, fill = drink_level), col = "black", alpha = 0.5) +
  labs(x = "", y = "frequency") +
  theme_bw() +
  theme(legend.position="", axis.text.x=element_text(angle=45, hjust=1))
```
...
Which cuisine is the best restaurant? (in terms of average rating)
